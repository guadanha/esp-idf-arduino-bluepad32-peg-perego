...
//https://github.com/ste2425/BluePad32OTAExample
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle,
    ...
    if (att_handle == ATT_CHARACTERISTIC_0000FF11_0000_1000_8000_00805F9B34FB_01_VALUE_HANDLE) {
        Console.printf("Responding right away\n");
...
static int att_write_callback(hci_con_handle_t connection_handle,
    ...
    if (att_handle == ATT_CHARACTERISTIC_0000FF11_0000_1000_8000_00805F9B34FB_01_VALUE_HANDLE) {
        ...
        if (buffer_size > 0) {
            Console.printf(" Writing Block\n");
            esp_err_t respr = esp_ota_write(otaHandler, buffer, buffer_size);

            if (respr != ESP_OK) {
                Console.printf(" Error writing OTA Block\n");
            }
            ...
            if (buffer_size != 244) {
                Console.printf(" Calling OTA End\n");

                if (ESP_OK != esp_ota_end(otaHandler)) {
                    Console.printf(" Error ending OTA\n");
                } else {
                    Console.printf(" OTA has finished\n");
                }
                ...
                if (ESP_OK == esp_ota_set_boot_partition(esp_ota_get_next_update_partition(NULL))) {
                    ...
                    Console.printf(" Setting boot Partition Error\n");
                }
            }
        }
    }
